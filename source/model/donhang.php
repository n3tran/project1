<?php
if (!function_exists('donhang_hoanthanh')) {
    function donhang_hoanthanh()
    {
...
if (!function_exists('loadall_donhang_update')) {
    function loadall_donhang_update()
    {
        $sql = "SELECT *  FROM `don_hang` 
        where trang_thai != 4
                group by ma_order";
...
if (!function_exists('insert_donhang')) {
    ...
    function insert_donhang($ho_ten, $email, $sdt, $dia_chi, $ghi_chu, $don_gia, $date_order, $trang_thai)
    {
        $conn = pdo_get_connection();
        $sql = "INSERT INTO `don_hang` (`ho_ten`, `email`, `sdt`, `dia_chi`, `ghi_chu`,`don_gia`,`ngay_dat`,`trang_thai`)
         VALUES ('$ho_ten', '$email', '$sdt', '$dia_chi', '$ghi_chu','$don_gia','$date_order','$trang_thai')";
        $conn->exec($sql);
